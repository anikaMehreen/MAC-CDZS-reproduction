...
class Class_Fin(nn.Module):
    def __init__(self, input_cn, output_cn):
        super(Class_Fin, self).__init__()
        self.layers = nn.Sequential(
            nn.Linear(input_cn, output_cn),
        )
    def forward(self, x):
...
class Projector(nn.Module):
    def __init__(self, input_cn, output_cn):
        ...
        self.layers = nn.Sequential(
            nn.Linear(input_cn, input_cn),
            nn.BatchNorm1d(input_cn),
            nn.ReLU(inplace=True),
            nn.Linear(input_cn, output_cn),
            nn.BatchNorm1d(output_cn),
        )
    def forward(self, x):
        return self.layers(x)
...
class FeatureExtractor_0(nn.Module):
    def __init__(self, input_nc=100, output_nc=128, avgPool=False):
        super(FeatureExtractor_0, self).__init__()
        self.conv_1 = nn.Sequential(
            nn.Conv2d(input_nc, output_nc // 4, kernel_size=3, stride=1, padding=2),
            nn.BatchNorm2d(output_nc // 4), nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
        self.conv_2 = nn.Sequential(
            nn.Conv2d(output_nc // 4, output_nc // 2, kernel_size=3, stride=1, padding=2),
...
class FeatureExtractor(nn.Module):
    def __init__(self, input_nc=100, output_nc=128, avgPool=False):
        ...
        self.conv_1 = nn.Sequential(
            nn.Conv2d(input_nc, output_nc, kernel_size=3, stride=1, padding=2),
            nn.BatchNorm2d(output_nc), nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
        self.conv_2 = nn.Sequential(
            nn.Conv2d(output_nc, output_nc * 2, kernel_size=3, stride=1, padding=2),
            nn.BatchNorm2d(output_nc * 2), nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1))
        if avgPool:
            self.conv_3 = nn.Sequential(
            ...
            self.conv_3 = nn.Sequential(
                nn.Conv2d(output_nc * 2, output_nc, kernel_size=3, stride=1, padding=1),
                nn.BatchNorm2d(output_nc), nn.ReLU())
...
class Classifier(nn.Module):
    def __init__(self, input_nc, output_nc, resnet=False):
        ...
        layers.append(nn.Linear(input_nc, input_nc))
...
class FeatureShifter_Att(nn.Module):
    ...
    def forward(self, att, x):
        ...
        x = self.attn2(att, x)
        return x
class CustomAttention_FC(nn.Module):
    def __init__(self, input_nc):
        super(CustomAttention_FC, self).__init__()
        output_nc = max(input_nc // 8, 1)
        self.query_conv = nn.Linear(input_nc, output_nc)
        self.key_conv = nn.Linear(input_nc, output_nc)
        self.value_conv = nn.Linear(input_nc, input_nc)
        self.gamma = nn.Parameter(torch.zeros(1))
        self.softmax = nn.Softmax(dim=-1)

    def forward(self, att, x):
        B, C, W, H = x.size()
        proj_query = self.query_conv(att.squeeze()).view(B, -1, W * H).permute(0, 2, 1)
        proj_key = self.key_conv(att.squeeze()).view(B, -1, W * H)
        energy = torch.bmm(proj_query, proj_key)
        attention = self.softmax(energy)
        proj_value = self.value_conv(x.squeeze()).view(B, -1, W * H)
        ...
        return out
class FeatureShifter_Att_FC(nn.Module):
    """Custom Attention based project module"""

    def __init__(self, input_nc, output_nc=None):
        super(FeatureShifter_Att_FC, self).__init__()
        if output_nc is None:
            output_nc = input_nc

        self.attn1 = CustomAttention_FC(input_nc)
        self.layer1 = nn.Sequential(nn.Linear(input_nc, output_nc),
                                    nn.LayerNorm(output_nc),
                                    nn.ReLU())

        self.attn2 = CustomAttention_FC(output_nc)

    def forward(self, att, x):
        x = self.attn1(att, x)
        x = self.layer1(x.squeeze())
        x = self.attn2(att, x.unsqueeze(2).unsqueeze(3))
        return x
...
def get_scheduler(optimizer, lr_policy='step'):
